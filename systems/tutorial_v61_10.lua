...
if type(queueBootstrap) == "function" then
    local queued =
        queueBootstrap("tutorial -> lobby")

    if queued then
        status("next teleport queued")
    else
        status("queue unavailable; re-execute once in Lobby if needed")
    end
end
...
if UnForgeUtil
    ...
    and type(UnForgeUtil.SelectDefaultWeapon)
        == "function"
then
...
elseif UnForgeUtil then
    ...
    status("weapon helper unavailable; still attempting tutorial skip")
end

-- Let starter state replicate before completing the tutorial.
task.wait(0.65)

if waitTeleport(0.25) then
    return
end
        ...
        if pg then
            ...
            for _, obj in ipairs(pg:GetDescendants()) do
                if looksLikeSkip(obj) then
                    ...
                    hiddenCandidate =
                        hiddenCandidate or obj
                end
            end

            if hiddenCandidate then
...
local function signalFire(signal)
    ...
    if type(firesignal) == "function" then
        if pcall(firesignal, signal) then
            return true
        end
    end
...
local function activateSkip(button)
    ...
    if signalFire(button.Activated) then
        return true, "ACTIVATED_SIGNAL"
    end

    if signalFire(button.MouseButton1Click) then
        return true, "MOUSE_CLICK_SIGNAL"
    end

    if signalFire(button.MouseButton1Down) then
...
if skip then
    ...
    status("Skip button not found; using direct Lobby fallback")
end

--========================================================--
-- CLIENT-SUPPORTED DIRECT LOBBY FALLBACK
--========================================================--
...
status(
    "STUCK | Lobby teleport did not start | "
        .. table.concat(directErrors, " | ")
)

warn(
    "[IronSoul V61.10] Tutorial could not start Lobby teleport: "
        .. table.concat(directErrors, " | ")
)
