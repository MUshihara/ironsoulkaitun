--========================================================--
-- IRON SOUL KAITUN - CONTINUOUS V59.3 QUIET BOOTSTRAP
--
...
-- Ignore multiple queued copies arriving in the same server at once.
do
    local key =
...
local function loadRaw(path)
    ...
    then
        warn(
            "[IronSoul V59.3] HTTP load failed: "
                .. tostring(path)
                .. " | "
                .. tostring(source)
        )
    ...
    if not normalized then
        warn(
            "[IronSoul V59.3] Source normalization failed: "
                .. tostring(path)
                .. " | "
                .. tostring(
                    normalizeErr
    ...
    if not fn then
        ...
        warn(
            "[IronSoul V59.3] SOURCE_HEAD="
                .. sourceHead(source)
        )

        warn(
            "[IronSoul V59.3] FIRST_BYTES="
                .. sourceBytes(source)
        )
...
local function queueBootstrap(reason)
    ...
    then
        warn(
            "[IronSoul V59.3] queue_on_teleport unavailable: "
                .. tostring(reason)
        )
...
then
    ...
    warn(
        "[IronSoul V59.3] Unknown place; no action. PlaceId="
            .. tostring(
                game.PlaceId
            )
            .. " WorldName="
